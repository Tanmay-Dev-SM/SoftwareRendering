...
{
	//--Sort the vertices by y-coordinates in ascending (y0 < y1 < y2)
	//--Right now Using Brute Force Approach
	if (y0 > y1) {
		int_swap(&y0, &y1);
		int_swap(&x0, &x1);
	}
	...
	if (y1 == y2) {
		//--Draw Only Upper Part
		fill_flat_bottom_triangle(x0, y0, x1, y1, x2, y2, color);
	}
	else if (y0 == y1) {
		//--Draw Only Lower Part
...
void draw_texel_affine(
	...
	// Interpolate UVs using affine weights
	...
	// Clamp final texel indices to texture bounds
	if (tex_x >= texture_width) tex_x = texture_width - 1;
	if (tex_y >= texture_height) tex_y = texture_height - 1;
...
void draw_texture_triangle(
	...
	//--Flip the V and U component to account for the inverted coordinates (our systems has V grows downwards)
	v0 = 1.0 - v0;
	v1 = 1.0 - v1;
	v2 = 1.0 - v2;
	...
	if (y1 - y0 != 0) {
		for (int y = y0; y <= y1; y++) {
			...
			for (int x = x_start; x < x_end; x++) {
				...
				draw_texel_affine(x, y, texture, a, b, c, a_uv, b_uv, c_uv);
			}
		}
	}
